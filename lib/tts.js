...
function arraySplit(text, lang)
{
	...
	for(var i = 0; i < splitList.length; i++) {
		...
		if(tmpStr.length + splitList[i].length > 80) {
			resultList.push({index: index, text:tmpStr});
			index++;
			tmpStr = splitList[i];
		} else {
...
tts.speakQueue = function (text, lang) {
	...
	async.each(textArray, function (obj, cb) {
		...
		var child = exec('/usr/bin/mplayer -really-quiet ' + filePrefix + '*.mp3',  function (error, stdout, stderr) {
			if(error)
				console.log(error);
